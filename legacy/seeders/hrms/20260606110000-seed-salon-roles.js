...
module.exports = {
  up: async (queryInterface) => {
    ...
    for (const role of ROLES) {
      ...
      if (existing) {
        await queryInterface.sequelize.query(
          `UPDATE ${schema}.roles SET hierarchy_level = :level, updated_at = :now WHERE id = :id`,
          { replacements: { level: role.hierarchy_level, now, id: existing.id } }
        );
      } else {
    ...
    // Rename legacy SuperAdmin if present
    await queryInterface.sequelize.query(
      `UPDATE ${schema}.roles SET name = 'SUPER_ADMIN', hierarchy_level = 0 WHERE name = 'SuperAdmin'`
    );
  },
